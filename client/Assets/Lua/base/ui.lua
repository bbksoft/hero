...
function UI.lock(v)
	if v then
		UI.lock_obj =  UITools.Show("Lock")
	elseif UI.lock_obj then
		GameObject.Destroy(UI.lock_obj.gameObject)
		UI.lock_obj = nil
	end
end

function UI.show_hint(str)
	UI.hint_obj =  UITools.Show("Hint")
	UITools.SetText(UI.hint_obj:FindChild("Text"),str)
end
...
function UI_display.text(node,ui,data)
	...
	if ui.text then
		...
		elseif t == "number" then
			value = format_number(data)
		else
			value = tostring(data)
		end

		if ui.color then
			UITools.SetColorText(node,value,ui.color)
		else
			UITools.SetText(node,value)
		end
	end
end
